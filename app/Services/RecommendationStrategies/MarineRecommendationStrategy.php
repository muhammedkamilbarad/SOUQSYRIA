...
class MarineRecommendationStrategy implements RecommendationStrategyInterface
{
    ...
    public function getSimilarAdvertisements(Advertisement $advertisement, Builder $query, int $limit): Collection
    {
        ...
        $marineType = $advertisement->marineAdvertisement->marine_type ?? null;
        ...
        if ($query->count() > 4000)
        {
            ...
            if ($marineType) {
                $query->whereHas('marineAdvertisement', function($q) use ($marineType) {
                    $q->where('marine_type', $marineType);
                });
            }
        ...
        // Combine results
        return $this->scoreCandidates($candidates, $advertisement, $brandId, $modelId, $year, $marineType, $limit);
    }
    
    private function scoreCandidates(Collection $candidates, Advertisement $advertisement, ?int $brandId, ?int $modelId, int $year, ?string $marineType, int $limit): Collection
    {
        return $candidates->map(function ($candidate) use (
            ...
            // Marine type match
            $candidateMarineType = $candidate->marineAdvertisement->marine_type ?? null;
            if ($marineType && $candidateMarineType && $candidateMarineType === $marineType) {
                $score += 35;
            }
